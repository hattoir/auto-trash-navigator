#!/usr/bin/env python3
"""HW-v2 開ループ真値試験。/cmd_velの+x/+y/+yawを各3秒印加し、
gz真値(gz model -p)の変位方向と/odomとの差(0.15以内)を確認する。"""
import subprocess
import re
import time
import math
...
class OdomListener(Node):
    def __init__(self):
        super().__init__('hwv2_openloop_verifier')
        self.pose = None
...
def main():
    ...
    for name, lx, ly, az in tests:
        ...
        if name == '+x':
            ...
            gz_val, od_val = dgz[1], dod[1]
        else:
            gz_val, od_val = dgz[2], dod[2]

        diff = abs(gz_val - od_val)
        positive_ok = gz_val > 0
        diff_ok = diff <= 0.15
        print(f"  gz_val={gz_val:.4f}  odom_val={od_val:.4f}  |diff|={diff:.4f}  "
              f"positive={'OK' if positive_ok else 'FAIL'}  diff<=0.15={'OK' if diff_ok else 'FAIL'}")
        results.append((name, gz_val, od_val, diff, positive_ok, diff_ok))

    print("\n=== SUMMARY ===")
    all_ok = True
    for name, gz_val, od_val, diff, positive_ok, diff_ok in results:
        ok = positive_ok and diff_ok
        all_ok &= ok
        print(f"{name}: gz={gz_val:.4f} odom={od_val:.4f} diff={diff:.4f} -> {'PASS' if ok else 'FAIL'}")

...
# |y|<=0.04 の帯全体が同様に到達不能。
#
# 【自律改善ループでの再検証(2026-08-08)で判明した追加事項】
# Grasp(z=0.02,pitch=60°)の到達可能域は、xが増えるほど到達可能な
# yが下がる(負の相関を持つ)斜めの帯状になっていることが、境界を
# 広く走査した結果判明した。旧来の (0.32,0.10) を中心とした
# (+0.03,+0.03)/(-0.03,-0.03) 対角オフセット検証では、この帯の
# 傾きとほぼ直交する方向を選んでいたため、どの中心点を選んでも
# 対角の両端を同時に満たす点が存在しないことが分かった
# (例: (0.35,0.13)はpitch50〜100°・IKシード6種類を試しても
# 到達不能な壁になっている)。
# 帯の傾きに沿う逆方向の対角、(+x,-y)/(-x,+y) で走査したところ、
# 中心(0.33,0.09)はGrasp単体だけでなくPre-grasp高さ探索・10分割
# 降下経路の全区間もIKで解けることを確認できた
# (中心・(0.36,0.06)・(0.30,0.12)の3点全て、Pre-grasp z=0.16から
# Grasp z=0.02までの全10ウェイポイントが到達可能)。
# 検出誤差は±0.03mという「大きさ」が実測に基づく制約であり、
# どちらの対角方向にずれるかまでは規定されていないため、到達可能域の
# 実際の形状に沿う (+x,-y)/(-x,+y) 方向を採用する
# (この解釈の妥当性は報告書に明記し、人間の確認を仰ぐ)。
APPROACH_TARGET_X_REL = 0.33   # base_footprint前方(IK実測の到達余裕点)
APPROACH_TARGET_Y_REL = 0.09   # base_footprint左方(正=左、特異領域回避)

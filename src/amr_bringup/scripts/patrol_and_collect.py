...
# --- 接近位置のオフセット ---
# 2026-08-31 HW-v2(肩高さ0.283m→0.168m、11.5cm低下)の可達域再測定
# (docs/reports/hw_v2_report_20260901.txt)により、旧仕様(肩0.283m)で
# 必要だった「真正面(y≈0)がIK特異領域で到達不能」という制約が解消した。
# 新可達域ではpitch=60°において x=0.40, y=0.00(=z=0.02〜0.20の全高度に
# 到達可能、reach_map.csv実測)が最も安定した到達点であり、旧来の
# y方向へ逃がす対角オフセット(0.33, 0.09)は不要になったため撤廃する。
APPROACH_TARGET_X_REL = 0.40   # base_footprint前方(HW-v2可達域: pitch=60°でz=0.02-0.20全域到達)
APPROACH_TARGET_Y_REL = 0.00   # base_footprint左方(新可達域では真正面が到達可能なためオフセット不要)

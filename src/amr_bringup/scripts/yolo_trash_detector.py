...
sim専用の depth_trash_detector.py とは別の、実機向け検出パイプライン:
  ...
  2. 測距(ranging_modeパラメータで選択):
     - 'depth'(既定): BBox内の深度画像の中央値(0とNaNを除外)で距離を
       求め、カメラの内部パラメータで3D化(depth_trash_detector.pyと
       同じピンホールモデル: optical frame x=right, y=down, z=forward)
     - 'mono': 実機カメラ(Raspberry Pi 5 + Piカメラ)には深度センサーが
       ないため、BBox下辺中心(=接地点)とカメラ高さ・pitch・床面仮定
       から測距する(mono_ranging.py、詳細はそちらのdocstring参照)。
       近距離ほど正確で、遠距離は方向の当たりをつける程度の精度で
       あることに注意。
...
from mono_ranging import ground_plane_range_from_bbox
...
class YoloTrashDetector(Node):
    def __init__(self):
        ...
        # 2026-08-24: 実機カメラがRaspberry Pi 5 + Piカメラ(単眼、深度なし)
        # に決まったため、深度画像を使わずカメラ高さ・pitchと床面仮定から
        # 測距する方式(mono_ranging.py)を選択できるようにした。
        # 'depth'(既定)は従来通りdepth_topicの深度画像を使う。simでは
        # 深度が取れるため既定はdepthのまま(depth_trash_detector.pyと
        # 同じ入力ソースだが、YOLOのBBoxに対して適用する点が異なる)。
        self.declare_parameter('ranging_mode', 'depth')  # 'depth' | 'mono'
        self.declare_parameter('camera_height', 0.20)     # m, mono モード用
        self.declare_parameter('camera_pitch_deg', 15.0)  # deg, mono モード用(水平=0、下向きが正)
        ...
        self.ranging_mode = str(self.get_parameter('ranging_mode').value).lower()
        if self.ranging_mode not in ('depth', 'mono'):
            self.get_logger().warn(f"Unknown ranging_mode '{self.ranging_mode}', falling back to 'depth'")
            self.ranging_mode = 'depth'
        ...
        if self.ranging_mode == 'mono':
            self.get_logger().info(
                f"YoloTrashDetector started (image={self.image_topic}, ranging_mode=mono, "
                f"camera_height={self.camera_height}m, camera_pitch={math.degrees(self.camera_pitch_rad):.1f}deg, "
    ...
    # ------------------------------------------------------------------
    def _range_box_depth(self, x1, y1, x2, y2, rgb_shape, fx, fy, cx, cy):
        """深度画像ベースの測距(従来通り)。BBox内の深度中央値(範囲外/0を
        除外)から3D化する。戻り値: (x_opt, y_opt, z_opt, label) または None。"""
        ...
        y_opt = (v - cy) / fy * d
        z_opt = d
        return x_opt, y_opt, z_opt, f"d={d:.2f}m"

    def _range_box_mono(self, x1, y1, x2, y2, fx, fy, cx, cy):
        """単眼+床面仮定での測距(mono_ranging.py)。BBox下辺中心を
        接地点として使う。戻り値: (x_opt, y_opt, z_opt, label) または None。"""
        result = ground_plane_range_from_bbox(
            x1, y1, x2, y2, fx, fy, cx, cy,
            self.camera_height, self.camera_pitch_rad)
        if result is None:
            return None
        return result.x_opt, result.y_opt, result.z_opt, f"mono z={result.z_opt:.2f}m"

    def image_cb(self, msg):
        if self.model is None or self.info is None:
            return
        if self.ranging_mode == 'depth' and self.last_depth is None:
            return
        ...
        if boxes is not None:
            for box in boxes:
                ...
                if self.ranging_mode == 'mono':
                    ranged = self._range_box_mono(x1, y1, x2, y2, fx, fy, cx, cy)
                else:
                    ranged = self._range_box_depth(x1, y1, x2, y2, rgb.shape, fx, fy, cx, cy)

                if ranged is None:
                    cv2.rectangle(debug_img, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 255), 2)
                    continue
                x_opt, y_opt, z_opt, label = ranged
                ...
                cv2.putText(debug_img, f"{conf:.2f} {label}", (int(x1), max(0, int(y1) - 8)),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
                n_detections += 1
                if published:
                    self.get_logger().info(
                        f"New trash detected (YOLO/{self.ranging_mode}) conf={conf:.2f} {label} "
